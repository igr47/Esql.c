...
class Token{
	public:
		enum class Type{
			//Keywords
			SELECT,FROM,WHERE,AND,OR,NOT,UPDATE,SET,DROP,TABLE,DELETE,INSERT,INTO,ALTER,CREATE,ADD,RENAME,VALUES,BOOL,TEXT,INT,FLOAT,DATABASE,DATABASES,SHOW,USE,TABLES,TO,ROW,BULK,IN,COLUMN,BETWEEN,
			//Identifier & Literals
			IDENTIFIER,STRING_LITERAL,NUMBER_LITERAL,DOUBLE_QUOTED_STRING,
			//conditiinals
			TRUE,FALSE,
			//OPERATORS
			EQUAL,NOT_EQUAL,LESS,LESS_EQUAL,GREATER,GREATER_EQUAL,ASTERIST,
			//Panctuation
			COMMA,DOT,SEMICOLON,L_PAREN,R_PAREN,COLON,
			//Special
			END_OF_INPUT,ERROR
		};
		Type type;
