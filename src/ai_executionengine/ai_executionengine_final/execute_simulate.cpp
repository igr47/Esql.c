...
ExecutionEngine::ResultSet AIExecutionEngineFinal::executeSimulate(
    ...
    try {
        ...
        simulator->set_mean_reversion_strength(stmt.mean_reversion_strength);
        ...
        if (stmt.plot_config.has_value() && !paths.empty()) {
            ...
            std::cout << "[AIExecutionEngineFinal] Starting professional real-time plotting..." << std::endl;

            auto path_data = std::make_shared<std::vector<esql::ai::SimulationPath>>(std::move(paths));

            // Create a dedicated thread for plotting
            std::thread([path_data, plot_config]() {
                try {
                    // Small delay to allow main thread to finish
                    std::this_thread::sleep_for(std::chrono::milliseconds(100));

                    std::cout << "[Plotting] Starting professional plotter thread" << std::endl;

                    auto plotter = std::make_unique<Visualization::ImPlotSimulationPlotter>();

                    // Initialize in this thread
                    plotter->initialize();
                    ...
                    if (plotter->isWindowClosed()) {
                        std::cerr << "[Plotting] Window setup failed" << std::endl;
                        return;
                    }
                    ...
                    std::cout << "[Plotting] Starting professional animation with "
                    ...
                    for (size_t step = 0; step < total_steps && !plotter->isWindowClosed(); ++step) {
                        ...
                        // Handle input for scrolling/zooming
                        plotter->handleInput();
                        ...
                        auto target_delay = plot_config.update_interval / plotter->getPlaybackSpeed();
                        auto sleep_time = target_delay - elapsed;
                    ...
                    std::cout << "[Plotting] Animation complete. Window will stay open for analysis." << std::endl;
            ...
            std::cout << "[AIExecutionEngineFinal] Professional plotting thread started." << std::endl;
        }
