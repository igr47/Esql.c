...
/*ExecutionEngine::ResultSet ExecutionEngine::executeAnalyticalSelect(AST::SelectStatement& stmt) {
    ...
    for (const auto& col : stmt.columns) {
        ...
        // Handle aliases for statistical functions
        if (auto* statExpr = dynamic_cast<const AST::StatisticalExpression*>(col.get())) {
            if (statExpr->alias) {
                colName = statExpr->alias->toString();
            }
        }
        // Handle aliases for other expressions
        else if (auto* caseExpr = dynamic_cast<const AST::CaseExpression*>(col.get())) {
            if (!caseExpr->alias.empty()) {
                colName = caseExpr->alias;
            }
        }
        // Handle window functions with aliases
        else if (auto* windowFunc = dynamic_cast<const AST::WindowFunction*>(col.get())) {
            //if (windowFunc->alias) {
                //colName = windowFunc->alias->toString();
            //}
    ...
    // Convert processed data to result format
    // For statistical functions, we need exactly ONE result row
    bool hasStatistical = hasStatisticalFunctions(stmt);
    bool hasWindow = hasWindowFunctions(stmt);

    if (hasStatistical && !hasWindow) {
        // Statistical functions without window functions: single result row
        if (!processedData.empty()) {
            std::vector<std::string> resultRow;
            for (const auto& col : stmt.columns) {
                std::string displayName = col->toString();

                // Get the value from the first row (all rows should have the same value)
                auto it = processedData[0].find(displayName);
                if (it != processedData[0].end()) {
                    resultRow.push_back(it->second);
                } else {
                    ...
                    if (auto* statExpr = dynamic_cast<const AST::StatisticalExpression*>(col.get())) {
                        if (statExpr->alias) {
                            ...
                            it = processedData[0].find(displayName);
                            if (it != processedData[0].end()) {
                                resultRow.push_back(it->second);
                                continue;
                            }
        ...
        for (const auto& row : processedData) {
            ...
            for (const auto& colName : result.columns) {
                ...
                if (it != row.end()) {
                    ...
                    // Try to find by expression string
                    for (const auto& col : stmt.columns) {
                        std::string exprStr = col->toString();
                        if (exprStr == colName) {
                            auto exprIt = row.find(exprStr);
                            if (exprIt != row.end()) {
                                resultRow.push_back(exprIt->second);
                                break;
                            }
                        }
                    }
                    if (resultRow.size() < result.columns.size()) {
                        resultRow.push_back("NULL");
                    }
                }
            }
...
/*ExecutionEngine::ResultSet ExecutionEngine::executeAnalyticalSelect(AST::SelectStatement& stmt) {
    ...
    filteredData = executeJoins(filteredData, stmt.joins);
     ...
     ResultSet result;
    std::vector<std::pair<std::string, std::unique_ptr<AST::Expression>>> columnMapping;

    // Check if we have regular aggregates
    bool hasAggregates = false;
    for (const auto& col : stmt.columns) {
        if (dynamic_cast<const AST::AggregateExpression*>(col.get())) {
            hasAggregates = true;
            break;
        }
    }
    ...
    // Build column mapping for non-grouped analytical queries
    for (const auto& col : stmt.columns) {
        std::string displayName = col->toString();

        // Handle aliases for case expressions
        ...
        columnMapping.emplace_back(displayName, col->clone());
    }
    ...
    if (stmt.groupBy) {
        ...
        for (const auto& col : stmt.groupBy->columns) {
            groupColumns.push_back(col->toString());
        }

        std::vector<std::unordered_map<std::string, std::string>> dataForGrouping;
        for (const auto& row : result.rows) {
            std::unordered_map<std::string, std::string> rowMap;
            for (size_t i = 0; i < result.columns.size(); ++i) {
                rowMap[result.columns[i]] = row[i];
            }
            dataForGrouping.push_back(rowMap);
        }

        //auto groupedData = groupRows(result.rows, result.columns, groupColumns);
        auto groupedData = groupRows(dataForGrouping, groupColumns);
        
        result.rows.clear();
        for (const auto& group : groupedData) {
            // Each group is a vector of rows - flatten it
            for (const auto& rowMap : group) {
                std::vector<std::string> rowVec;
                for (const auto& colName : result.columns) {
                    rowVec.push_back(rowMap.at(colName));
                }
    ...
    if (stmt.having) {
        ...
        for (const auto& row : result.rows) {
            ...
            if (evaluateHavingCondition(stmt.having->condition.get(), rowMap)) {
