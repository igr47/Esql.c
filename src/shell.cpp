...
void ESQLShell::redraw_interface() {
    ...
    get_terminal_size();
    int prompt_length = get_current_time().length() + current_db.length() + 8; // "[HH:MM] • db> "

    // Calculate available space for input
    int available_width = terminal_width - prompt_length - 1; // -1 for safety margin
    ...
    // Handle line wrapping for long input
    std::string display_line = current_line;
    std::string colored_line = colorize_sql(display_line);

    // If line is too long, we need to handle wrapping
    if (display_line.length() > available_width) {
        // Find where to break the line for display
        int display_cursor_pos = cursor_pos;

        // If cursor is beyond available width, scroll the display
        if (cursor_pos >= available_width) {
            int scroll_offset = cursor_pos - available_width + 10; // Keep some context
            if (scroll_offset < 0) scroll_offset = 0;
            if (scroll_offset + available_width > display_line.length()) {
                scroll_offset = display_line.length() - available_width;
            }
            if (scroll_offset < 0) scroll_offset = 0;

            display_line = display_line.substr(scroll_offset, available_width);
            colored_line = colorize_sql(display_line);
            display_cursor_pos = cursor_pos - scroll_offset;
        } else {
            // Just truncate for display if too long but cursor is in visible area
            display_line = display_line.substr(0, available_width);
            colored_line = colorize_sql(display_line);
        }

        std::cout << colored_line;

        // Position cursor correctly within visible area
        if (display_cursor_pos > available_width) {
            display_cursor_pos = available_width;
        }
        std::cout << "\033[" << (prompt_length + display_cursor_pos) << "G";
    } else {
        // Normal case - line fits in terminal
        std::cout << colored_line;
...
/*std::vector<std::string> ESQLShell::load_ascii_art() {
 ...
 YELLOW"                    ...  .:...." RESET,
 YELLOW"         ..,l:c,'.....,;;;,.. " RESET,
 YELLOW"      ..ccoo;'...      .',ll::.." RESET,
 YELLOW"     .,xxodc..          .,:dodl,."RESET,
 YELLOW"   ..clkkoo,             .;oddxl;'."RESET,
 YELLOW"  .,:ddxxkd,.            .cddddocc:."RESET,
 YELLOW"  ';ooxllkx:             .lxodoxll:."RESET,
 YELLOW" ..cldolclxl.  . ..     .,xooldooc:'"RESET,
 YELLOW" .,;cllc:;cdd, .:xkl,..,ddc::looolc'"RESET,
 YELLOW"  ';loll:,,,:xl.'do:..ld:;,;cloooc'"RESET,
 YELLOW"   .,coooc:,,;lododxddc,,,;coool;."RESET,
 YELLOW"     ..;clll:;,,lc:::,,;:cllc:,."RESET,
 YELLOW"        ..,;::c;;:,,c,::cc,..."RESET,
 YELLOW"         ....',,::,:cc:,'..."RESET,
 YELLOW"         ;,',;::c;,,ccc::;'."RESET,
 YELLOW"       ,':;,;:cccll:ccl:;''''.."RESET,
 YELLOW"       .,,':,;ccl:c;:::,,;..,;"RESET,
 YELLOW"       ....',,;c:;;;:;;;;.'.'."RESET,
 YELLOW"         ....','.......''...."RESET
    };
...
void ESQLShell::print_banner() {
    ...
    if (use_colors) {
        ...
        // Ensure we have enough lines for both banners
        size_t max_lines = std::max(phoenix_art.size(), size_t(6)); // ESQL has 6 lines

        // Print side-by-side banners
        for (size_t i = 0; i < max_lines; ++i) {
            // Phoenix ASCII art (left side)
            if (i < phoenix_art.size()) {
                std::cout << phoenix_art[i];
            } else {
                // Fill empty space if phoenix art has fewer lines
                std::cout << std::string(40, ' ');
            }

            // ESQL banner (right side)
            switch (i) {
                case 0:
                    std::cout << "   " << GRAY << "███████╗███████╗ ██████╗ ██╗   " << RESET;
                    break;
                ...
                case 5:
                    ...
                    break;
                default:
                    // For any additional lines from phoenix art
                    std::cout << "                                      ";
                    break;
            }
            std::cout << "\n";
